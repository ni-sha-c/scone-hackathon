...
def pde(tar_sc , model, x):
    ...
    dy = lambda x : torch.func.jacrev(model)(x).squeeze()
    dy_dotq = lambda x : torch.dot(dy(x), tar_sc(x))
    d_dy_dotq = lambda x : torch.func.jacrev(dy_dotq)(x)
    divergence_dy = lambda x : torch.trace(torch.func.jacrev(dy)(x))
    d_divergence_dy = lambda x : torch.func.jacrev(divergence_dy)(x)
    ...
    with progressbar.tqdm (range(epochs), unit="epoch") as pbar:
        for epoch in range(epochs):
            ...
            for t, (x, y) in enumerate(dataloader):
                ...
                x_g, y_g = x, y
                # y_g = y_g.unsqueeze(1)
                output = torch.empty((batchsize, 2), requires_grad=True, device = device)
